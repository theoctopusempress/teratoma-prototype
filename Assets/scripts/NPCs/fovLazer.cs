...
public class fovLazer : MonoBehaviour
{
    ...
    void Update()
    {
       ...
       middlePoint = Line.GetPosition(1);
            ...
            var eyeBalls = witness.GetComponent<Securitycamera>();
            Vector3 right45;
            Vector3 left45;
            if (eyeBalls == null){
                right45 = (witness.transform.up + witness.transform.right).normalized;
                left45 = (witness.transform.up - witness.transform.right).normalized;
            }
            else {
                 right45 = eyeBalls.currentRotation * (witness.transform.up + witness.transform.right).normalized;
                left45 = eyeBalls.currentRotation * (witness.transform.up - witness.transform.right).normalized;
            }
        Quaternion spreadAngle = Quaternion.AngleAxis(-45, witness.transform.up);
        //Vector2 targetDirection = spreadAngle * noAngle;
        RaycastHit2D hit = Physics2D.Raycast(middlePoint, right45, sightDistance, ~IgnoreThis);
        ...
        RaycastHit2D hit2 = Physics2D.Raycast(middlePoint, left45, sightDistance, ~IgnoreThis);
